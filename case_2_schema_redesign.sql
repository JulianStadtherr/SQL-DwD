...
CREATE SCHEMA pizza_runner; -- There can be several Schema in 1 DB. They can cluster areas (ex sales)
SET search_path = pizza_runner; -- Lets postgres know which schema we are in for the rest of (only) this session. For a permanent setting, write ALTER DATABASE your_database_name SET search_path TO pizza_runner;
