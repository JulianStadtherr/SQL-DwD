...
(SELECT
    ...
    s.order_date, 
        ...
        OVER(PARTITION BY customer_id ORDER BY s.order_date ASC) --View
...
JOIN dannys_diner.menu as m ON s.product_id = m.product_id) AS ord_rnk
WHERE order_rank = 1;
...
GROUP BY s.customer_id, m.product_name --! all columns that are in select but not aggregate columns must be written in GROUP BY
) as ord
WHERE row_number = 1;

customer_id	product_name	orders_total	row_number
A	ramen	3	1
B	ramen	2	1
C	ramen	3	1
...
customer_id	join_date	order_date	product_name	order_rank
...
customer_id	join_date	order_date	product_name	order_rank
A	2021-01-07	2021-01-01	sushi	1
...
FROM dannys_diner.sales as s
JOIN dannys_diner.menu as m ON s.product_id = m.product_id
FULL JOIN dannys_diner.members as mb ON s.customer_id = mb.customer_id
ORDER BY customer_id --! Do not use GROUP BY here or it could sum up rows (ex. with identical date if grouped by date). Only use GROUP BY when an agg function is used
;
...
SELECT 
    ...
    THEN 
    ROW_NUMBER()
    	OVER(PARTITION BY s.customer_id, s.order_date >= mb.join_date ORDER BY s.order_date) --! Must include s.order_date >= mb.join_date to start at 1 where condition is met
    END 
    AS rank
...
customer_id	order_date	product_name	price	members	rank
